...
The weighed sum of the inputs is then passed through a stepping function, that triggers the activation of an output should a threshold be surpassed. Assuming for the purposes of this example that the threshold is zero, and the stepping function is the Heaviside function, the output of a neuron can be computed as:
...
Finally, concatenating nodes in a forward fashion creates the concept of levels, or \emph{layers} in a network. When all neurons in a layer are fully connected to the nodes in the previous level, this is typically named a \emph{Dense} layer. Arranging more than one dense layers in series results in a NN.
